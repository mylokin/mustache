...
class TestPartials(unittest.TestCase):
    ...
    def test_partials_1(self):
        template = u'"{{>text}}"'
        template = mustache.build(template, partials={})
        context = {}
        rendered = mustache.render(template, context)
    ...
    def test_partials_2(self):
        template = u'"{{>partial}}"'
        template = mustache.build(template, partials={u'partial': u'*{{text}}*'})
    ...
    def test_partials_3(self):
        template = u'{{>node}}'
        template = mustache.build(template, partials={u'node': u'{{content}}<{{#nodes}}{{>node}}{{/nodes}}>'})
