...
class TestPartials(unittest.TestCase):
    def test_partials_0(self):
        template = u'"{{>text}}"'
        template = mustache.process(template, partials={u'text': u'from partial'})
        context = {}
    ...
    def test_partials_1(self):
        template = u'"{{>text}}"'
        template = mustache.process(template, partials={})
        context = {}
        rendered = mustache.render(template, context)
    ...
    def test_partials_2(self):
        template = u'"{{>partial}}"'
        template = mustache.process(template, partials={u'partial': u'*{{text}}*'})
    ...
    def test_partials_3(self):
        template = u'{{>node}}'
        template = mustache.process(template, partials={u'node': u'{{content}}<{{#nodes}}{{>node}}{{/nodes}}>'})
    ...
    def test_partials_4(self):
        template = u'| {{>partial}} |'
        template = mustache.process(template, partials={u'partial': u'\t|\t'})
        context = {}
        rendered = mustache.render(template, context)
        self.assertEqual(rendered, u'| \t|\t |')
...
class TestInverted(unittest.TestCase):
    def test_inverted_0(self):
        ...
        template = mustache.process(template, partials=None)
